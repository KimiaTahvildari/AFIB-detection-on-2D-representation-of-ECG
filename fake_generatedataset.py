...
class TrainProbeGenerator:
    def __init__(self, signal_path=None):
        ...
        self.list_of_segments = self.split_dataframe_into_smaller(
            self.dataframe,
    ...
    def show_scalogram(segment):
        ...
        coeffs, freqs = pywt.cwt(segment, scales, 'mexh')
        plt.imshow(coeffs, extent=[-1, 1, 1, 31], cmap=cm.gray, aspect='auto')
        plt.axis('off')
        plt.margins(0, 0)
        return fig
    ...
    def show_attractor(segment):
        ...
        try:
            ...
            k = gaussian_kde([v, w])
            xi, yi = np.mgrid[
                v.min():v.max():nbins*1j,
                w.min():w.max():nbins*1j
            ]
...
labels = ['AFIB', 'NORMAL']
plot_types = ['spectrogram', 'scalogram', 'attractor']

for lbl in labels:
    for ptype in plot_types:
        os.makedirs(os.path.join(output_root, lbl, ptype), exist_ok=True)
...
for fname in tqdm(os.listdir(base_dir)):
    ...
    for seg in tqdm(gen.list_of_segments):
        ...
        fig1.savefig(
            os.path.join(output_root, label, 'spectrogram', f"{label}_{idx}.jpg"),
        ...
        fig2.savefig(
            os.path.join(output_root, label, 'scalogram', f"{label}_{idx}.jpg"),
            bbox_inches='tight', pad_inches=0, dpi=100
        )
        plt.close(fig2)
        ...
        if fig3 is not None:
            fig3.savefig(
                os.path.join(output_root, label, 'attractor', f"{label}_{idx}.jpg"),
                bbox_inches='tight', pad_inches=0, dpi=100
            )
            plt.close(fig3)